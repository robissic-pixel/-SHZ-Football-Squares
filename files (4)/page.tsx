...
function BoardPanel({ boardKey, theme }: { boardKey: BoardKey; theme: Theme }) {
  ...
  const doResetBoard = async () => {
    ...
    try {
      ...
      // Silent reload: updates squares/digits/quarters/settings in place
      // without flipping the whole panel back to the full-screen loading
      // state. resetBoard() never touches team names, so a non-silent
      // load() here would just make the Teams panel (and everything else)
      // disappear behind "Loading the board…" for a moment and then
      // reappear with the same names — which reads as a "team name reset"
      // even though nothing about the settings actually changed.
      load(true);
    } catch (e: any) {
